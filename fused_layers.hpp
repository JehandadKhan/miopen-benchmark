...
struct FusedCNRLayer: public ConvDesc, public ConvLayerDesc, public Layer {
    ...
    virtual std::ostream& write_name(std::ostream& os) const 
    {
        ...
        return os << "FusedCNRLayer(" << kernel_size << "x" << kernel_size << ",pad=" << padding << ",s=" << stride << ",m=" << mode << "is_fused_faster" << is_fused_faster << ")";
    }
    ...
    void init_forward(const Tensor& input, Tensor& output) override
    {
        ...
        auto fused_time = lt.avg_time_ms();
...
struct FusedBNR: public Layer {
    ...
    {
        //return os << "Conv(" << kernel_size << "x" << kernel_size << ")";
        return os << "FusedBNR(" << "is_fused_faster=" << is_fused_faster << ",bn_time" << 
        bn_time << ",rlu_time" << rlu_time << ",disc_time" << disc_time << ",fused_time" << fused_time << ")";
    }
                  ...
                  running_mean(bn_dim), running_var(bn_dim), epsilon(eps)
    {
      // create the fusion plan and 
       ...
       model = std::make_shared<Sequential>(input_dims, "FusedBNR_discrete");
       model->emplace<BatchNormInference>();
       model->emplace<ReLU>();
    }
    ...
    void init_forward(const Tensor& input, Tensor& output) override
    {
        ...
        fused_time =  lt.avg_time_ms();

                //Time the discrete layers
        #if 0
        std::vector<layertimer> v;
        v.emplace_back("bn");
        v.emplace_back("relu");

        for(auto iter_idx = 0; iter_idx < iters; iter_idx++)
        {
          for(auto idx = 0; idx < model->layers.size(); idx++)
          {
            v[idx].tic();
            model->forward_step(input, output, idx);
            v[idx].toc();
          }
        }

        bn_time = v[0].avg_time_ms();
        rlu_time = v[1].avg_time_ms();
        #endif
